...
app.get('/api/monitor/repairs/:storeId', (req, res) => {
  ...
  const repairs = db.prepare(`SELECT r.ticket_number, r.device_brand, r.device_model, r.status, r.priority,
    ...
    ORDER BY CASE r.priority WHEN 'urgent' THEN 1 WHEN 'high' THEN 2 WHEN 'normal' THEN 3 ELSE 4 END,
    r.created_at DESC LIMIT 20`).all(req.params.storeId);
...
app.get('/api/transfers', authMiddleware, (req, res) => {
  ...
  const transfers = db.prepare(`SELECT st.*, p.name as product_name, p.sku,
    ...
    ${whereClause}
    ORDER BY st.created_at DESC LIMIT 100`).all(...params);
  res.json(transfers);
});
...
app.post('/api/print/receipt', authMiddleware, (req, res) => {
  ...
  const itemsHtml = (sale.items || []).map(item =>
    `<tr><td>${item.product_name || item.name}</td><td style="text-align:center">${item.quantity}</td><td style="text-align:right">\u00A3${(item.unit_price || 0).toFixed(2)}</td><td style="text-align:right">\u00A3${(item.total || 0).toFixed(2)}</td></tr>`
  ).join('');
  const html = `<!DOCTYPE html><html><head><style>body{font-family:'Courier New',monospace;width:280px;margin:0 auto;font-size:12px;}.center{text-align:center;}table{width:100%;border-collapse:collapse;}td{padding:2px 0;}.line{border-top:1px dashed #000;margin:5px 0;}@media print{body{margin:0;}}</style></head><body><div class="center"><strong>PEACEPLANET</strong><br>${sale.store_name||''}<br><br></div><div>Receipt: ${sale.receipt_number}<br>Date: ${new Date(sale.created_at||Date.now()).toLocaleString('en-GB')}<br>Cashier: ${sale.cashier_name||''}</div><div class="line"></div><table><tr><td><strong>Item</strong></td><td style="text-align:center"><strong>Qty</strong></td><td style="text-align:right"><strong>Price</strong></td><td style="text-align:right"><strong>Total</strong></td></tr>${itemsHtml}</table><div class="line"></div><table><tr><td><strong>TOTAL:</strong></td><td style="text-align:right"><strong>\u00A3${(sale.total||0).toFixed(2)}</strong></td></tr><tr><td>Paid (${sale.payment_method||'cash'}):</td><td style="text-align:right">\u00A3${(sale.total||0).toFixed(2)}</td></tr></table><div class="line"></div><div class="center">Thank you for shopping at PeacePlanet!</div><script>window.onload=function(){window.print();}<\/script></body></html>`;
  res.json({ html });
});
